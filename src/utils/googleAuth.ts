import { useState } from 'react';
import { Platform } from 'react-native';
import { api, setToken } from '../services/api';

const isWeb = Platform.OS === 'web';

const GOOGLE_CLIENT_ID = '656427464240-lqo51qasd990ejpct9d723n7kf5fe2eq.apps.googleusercontent.com';

export function useGoogleAuth(
  inviteCode?: string,
  flatNumber?: string,
  onLogin?: () => void
) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  if (isWeb) {
    const { useGoogleLogin } = require('@react-oauth/google');
    const googleLogin = useGoogleLogin({
      onSuccess: async (response: any) => {
        setLoading(true);
        setError(null);
        try {
          const token = response.credential || response.access_token || response.code;
          if (!token) { setError('Google token alınamadı'); setLoading(false); return; }
          const res = await api.auth.googleLogin(token, inviteCode, flatNumber);
          await setToken(res.token);
          onLogin?.();
          window.location.reload();
        } catch (e: any) {
          setError(e.message || 'Google girişi başarısız');
        } finally { setLoading(false); }
      },
      onError: () => { setError('Google girişi iptal edildi'); setLoading(false); },
    });
    return { googleLogin, loading, error };
  }

  return {
    googleLogin: () => setError('Google girişi sadece web\'de kullanılabilir'),
    loading: false,
    error: null,
  };
}

export { GOOGLE_CLIENT_ID };
